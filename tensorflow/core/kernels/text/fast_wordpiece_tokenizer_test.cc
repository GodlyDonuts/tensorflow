...
#include "tensorflow/core/kernels/text/fast_wordpiece_tokenizer.h"
...
TEST(FastWordpieceTokenizerTest, LoadAndTokenize) {
  ...
  // TODO(gasanov): validate it in open source
...
TEST_P(TestPunctuationVersionMismatch, Test) {
  ...
  auto status = tensorflow::ReadFileToString(
      tensorflow::Env::Default(),
      tensorflow::GetDataDependencyFilepath(kTestConfigUnicodePath),
      &config_flatbuffer);
  ASSERT_TRUE(status.ok());
...
TEST_P(TestTokenizeDetokenize, Test) {
  ...
  // Test detokenization.
  TF_ASSERT_OK_AND_ASSIGN(auto output_text,
