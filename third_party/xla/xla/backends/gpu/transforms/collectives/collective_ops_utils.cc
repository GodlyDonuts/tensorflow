...
absl::StatusOr<CollectiveOpSpec> ExtractCollectiveOpSpec(
  ...
  const HloInstruction* core_inst = inst;
  if (inst->opcode() == HloOpcode::kFusion) {
    for (const auto* fused_inst :
         Cast<HloFusionInstruction>(inst)->fused_instructions()) {
      if (HloAllReduceInstructionBase::ClassOf(fused_inst)) {
        core_inst = fused_inst;
        break;
      }
    }
  }
  if (auto* all_reduce = DynCast<HloAllReduceInstructionBase>(core_inst)) {
  ...
  return absl::InvalidArgumentError(
      absl::StrCat("Instruction is not a collective: ", inst->ToString()));
}
